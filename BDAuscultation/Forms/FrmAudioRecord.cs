...
namespace BDAuscultation.Forms
{
    public partial class FrmAudioRecord : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            string sqlInsertAudio = @"insert into AudioInfo(GUID,PGUID,StetName,Part,TakeTime,RecordTime,AudioGroupID)
                ...
                foreach (DataGridViewRow dr in dataGridViewEx1.Rows)
                {
                    ...
                    if (q.Equals("0"))
                    {
                        var d = Mediator.sqliteHelper.ExecuteNonQuery(sqlInsertAudio, guid, PatientGUID,
                            ...
                            recordTime, Setting.authorizationInfo.GroupID);
                    }
    ...
    PatientGroupID, PatientID, PatientName, PatientSex, PatientAge, DocName, DocDiagnose, DocRemark, His);
        ...
        public bool CanEdit
        {
            ...
            set
            {
                ...
                txtDocDiagnose.BorderColor = Color.Blue;
                if (string.IsNullOrEmpty(txtDocName.Text))
                    txtDocName.BorderColor = Color.Blue;
                txtDocRemark.BorderColor = Color.Blue;
    ...
    PatientGroupID, PatientID, PatientName, PatientSex, PatientAge, DocName, DocDiagnose, DocRemark, His);
                ...
                #endregion
                //MessageBox.Show("添加成功");
                Mediator.WriteLog(this.Name,"添加成功");//test
            }
            CanEdit = false;
            ...
            {
                Mediator.WriteLog(this.Name,"开始上传");//test
    ...
    PatientGroupID+"", PatientID, PatientName,PatientSex,PatientAge+"", DocName, DocDiagnose, DocRemark,His});
                ...
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    string sqlInsertAudioInfo = "exec P_UpLoadAudioInfo {0},{1},{2},{3},{4},{5},{6}";
                    var n = Mediator.remoteService.ExecuteNonQuery(sqlInsertAudioInfo, new string[]{GUID, PGUID, StetName , Part, TakeTime+"" 
  , RecordTime.ToString("yyyy-MM-dd HH:mm:ss"), Setting.authorizationInfo.GroupID.ToString()});
                    if (n > 0)
                    {
                        string filePath = Path.Combine(Setting.localData, @"DevicesData\AudioFiles\" + RecordTime.Year
                 + "\\" + RecordTime.Month + "\\" + RecordTime.Day + "\\" + GUID + ".MP3");
                    ...
                    {
                        Mediator.ShowMsg(string.Format("文件 {0} 上传失败...", GUID));//test
                    }
                }
                #endregion
